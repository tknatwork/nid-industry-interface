...
import { listPaymentCases, type PaymentCase } from '@nid/module-admin-accountability';
import { rupees } from '~/lib/money';
import { decidePaymentAction } from './actions';

export const metadata: Metadata = {
  title: 'Payment cell · Admin · NID Industry Interface',
  robots: { index: false, follow: false },
};

export default async function PaymentCellPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  const error = (await searchParams).error;
  const cases = listPaymentCases();
  const open = cases.filter((c) => c.status === 'open');
  const decided = cases.filter((c) => c.status !== 'open');

  return (
    <AdminShell activeNav="payment-cell" roleLabel="Payment cell · NID Ahmedabad">
      <section style={{ paddingInline: 'var(--layout-page-x)', paddingBlock: 'var(--space-10)' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          <header style={{ marginBottom: 'var(--space-6)' }}>
            <p style={labelS}>Accountability</p>
            <h1 style={h1}>Payment cell</h1>
            <p style={{ fontSize: 'var(--fs-14)', color: 'var(--text-secondary)', marginTop: 'var(--space-2)' }}>
              Refund + dispute adjudication (Phase 5.15). The participation fee is non-refundable except for
              institution-cancelled cycles, duplicate payments, or force majeure. Decisions are final.
            </p>
          </header>
          ...
          {open.length === 0 ? (
            <p style={notice}>No open payment cases.</p>
          ) : (
                ...
                <div key={c.id} style={card}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--space-2)' }}>
                    <p style={{ fontSize: 'var(--fs-16)', fontWeight: 'var(--fw-500)', color: 'var(--text-strong)' }}>
                      {c.companyName} · {c.kind}
                    </p>
                    <StatusPill tone="warning">open</StatusPill>
                  </div>
                  <p style={{ fontSize: 'var(--fs-14)', color: 'var(--text-strong)', marginTop: 'var(--space-1)' }}>{c.category}</p>
                  <p style={{ fontSize: 'var(--fs-12)', color: 'var(--text-secondary)' }}>
                    {c.amountPaise !== undefined ? `${rupees(c.amountPaise)} · ` : ''}filed {fmt(c.filedAt)}
                  </p>
                  <form action={decidePaymentAction} style={{ marginTop: 'var(--space-3)', display: 'flex', gap: 'var(--space-2)', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
            {decided.map((c) => (
              <div key={c.id} style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--space-2)' }}>
                  <p style={{ fontSize: 'var(--fs-14)', fontWeight: 'var(--fw-500)', color: 'var(--text-strong)' }}>{c.companyName} · {c.kind} · {c.category}</p>
...
function statusTone(s: PaymentCase['status']): StatusTone {
